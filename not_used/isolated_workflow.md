...
Companion documents: `drilling_workflow.md` (the isolation methodology this plan follows) and
...
### 8.4 Every stage artifact is parked
...
Respondent × product rows, **all** attributes, `K` from turn 2. The reference implementation
(k-means++ init, `n_init=10`, empty-cluster repair, best-inertia selection) is sound; adopt it with
the §13 fixes. **Pass an explicit `random_state`** and report it — verified reproducible with a
...
**Step 4 — turn 1, encoding (§9.2)**, run with `encoding="auto"` throughout.
...
**Step 5 — turn 2 + stages G, D, F, I, J, K.**
...
**Step 7 — turn 3 (§9.4).**
*Accept when:* every figure in the report traces to a stage output; the KPI direction appears in
words; `STILL MISSING` is present and lands in `analysis_missing`.

**Step 8 — mirror into `deployment/`.** `isolated_workflow_instructions.py` and the workflow module
