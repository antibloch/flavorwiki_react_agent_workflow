"""Reference client for api_funda_agent_exp.py -- prints the answer as it is generated.
    ...
    FLAVORAI_API=https://<name>.ngrok-free.dev python scripts/ask_stream.py "..."
...
def main():
    ...
    with requests.post(url, json=body, headers=HEADERS, stream=True, timeout=900) as r:
        r.raise_for_status()
            ...
            if kind == "token":
                ...
                if not a.quiet:
                    ...
                    print(f"[{ev['wall_s']}s · {ev['turns']} turns · in {t['input']} "
                          f"(cached {t['cached_input']}) · out {t['output']}]", file=sys.stderr)
